...
uses
  ...
  Dialogs, Grids, ExtCtrls, frmDiffMain, ShellApi, ShlObj, DirWatch, dxBar;
...
begin
  ...
  fDirectoryWatch1 := TDirectoryWatch.Create(TDiffMainFrm(Application.MainForm.ActiveMDIChild));
  fDirectoryWatch1.OnChange := DirectoryWatchOnChange;
  fDirectoryWatch2 := TDirectoryWatch.Create(TDiffMainFrm(Application.MainForm.ActiveMDIChild));
  ...
  sgFolder1.ColWidths[1] :=
    TDiffMainFrm(Application.MainForm.ActiveMDIChild).Canvas.TextWidth(format('    %1.0n KB',[1024 * 99000/1]));
  sgFolder2.ColWidths[1] := sgFolder1.ColWidths[1];

  sgFolder1.ColWidths[2] :=
    TDiffMainFrm(Application.MainForm.ActiveMDIChild).Canvas.TextWidth(formatDatetime(shortDateFormat +
      '        '+ ShortTimeFormat, 0));
...
begin
  ...
  begin
    tbFolder.ImageIndex := frmDiffMain.FOLDERVIEW;
    ...
    mnuCopyBlockLeft.Visible := ivNever;
    mnuCopyBlockRight.Visible := ivNever;
    //N1.Visible := false;
    mnuCompareFiles.Visible := ivAlways;
    mnuCompareFiles.OnClick := sgFolder1DblClick;
    mnuNext.visible := ivNever;
    mnuPrev.visible := ivNever;
...
begin
  ...
  if IsFolder1 then
  begin
    if not GetFolder(TDiffMainFrm(Application.MainForm.ActiveMDIChild),'Open Folder 1',LastOpenedFolder1) then exit;
    DoOpenFolder(LastOpenedFolder1, true)
  end else
  begin
    if not GetFolder(TDiffMainFrm(Application.MainForm.ActiveMDIChild),'Open Folder 2',LastOpenedFolder2) then exit;
...
begin
  ...
  TDiffMainFrm(Application.MainForm.ActiveMDIChild).Statusbar1.Panels[3].text := '';
...
begin
    ...
    TDiffMainFrm(Application.MainForm.ActiveMDIChild).mnuCompare.Enabled := false;
    TDiffMainFrm(Application.MainForm.ActiveMDIChild).tbCompare.Enabled := false;
...
begin
  ...
  TDiffMainFrm(Application.MainForm.ActiveMDIChild).Statusbar1.Panels[3].text := fDiffResultStr;
end;
//------------------------------------------------------------------------------
...
begin
  ...
  TDiffMainFrm(Application.MainForm.ActiveMDIChild).mnuCompareFiles.Enabled :=
...
begin
  ...
  if assigned(fFolder1StringList.objects[i]) and
    ...
    begin
      //toggle to fileview ...
      TDiffMainFrm(Application.MainForm.ActiveMDIChild).mnuFolderClick(nil);
      //check for overwriting of opened files ...
      with TFilesFrame(TDiffMainFrm(Application.MainForm.ActiveMDIChild).FilesFrame) do
      begin
        ...
        begin
          case application.MessageBox(pchar('Save changes to'#10 +
            ...
            IDCANCEL: exit;
            IDYES: SaveFileClick(TDiffMainFrm(Application.MainForm.ActiveMDIChild).mnuSave1);
          end;
        end;
        ...
        begin
          case application.MessageBox(pchar('Save changes to'#10 +
            pnlCaptionRight.Caption+' ?  '),pchar(application.title),
              MB_YESNOCANCEL or MB_ICONSTOP or MB_DEFBUTTON3) of
            IDCANCEL: exit;
            IDYES: SaveFileClick(TDiffMainFrm(Application.MainForm.ActiveMDIChild).mnuSave2);
          end;
        end;
      ...
      TDiffMainFrm(Application.MainForm.ActiveMDIChild).mnuCompare.Click;
    end;
end;
//------------------------------------------------------------------------------
